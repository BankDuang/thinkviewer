...
A self-hosted, browser-based remote desktop application (like TeamViewer), presented as a **macOS-style desktop web UI**. A Python/FastAPI **engine** does the host-OS work (screen capture, input injection, PTY terminals, files, auth); a **React + Vite + TypeScript SPA** (`frontend/`) renders a draggable-window desktop with a dock, menu bar, wallpapers, and six apps: Remote Desktop, Terminal, Files, Settings, **Servers** (a process manager for sibling Python apps under `~/Desktop/public_server`), and **Client Project** (a CRM + project-tracker for a software house — clients/projects/phases/tasks/issues/change-requests/meeting-notes/requirements/files/payments + dashboards, where a project can link to a managed service in Servers via `server_service`). Real-time data is WebSocket-first.
